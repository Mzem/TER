...
//initialise un polynome (degré=deg et coef=0)
...
void get_polynomial_coef (mpz_t* p_coef,Polynomial* p_poly, unsigned int order)
{
  if (order > p_poly->deg) {
...
int testAKS(const mpz_t n)
{
  ...
  while (mpz_cmp(r, n) < 0) {
    ...
    if (sieve_primality_test(r_ui, &sieve) == 1) {//a modifier
      int is_break = 0;
      mpz_t pwm;
      mpz_init(pwm);
      mpz_t i;
  ...
  while (mpz_cmp(a, amax) <= 0) {
    ...
    polynomial_modular_power(&p_poly_left, p_poly_left_base, n, r_ui);//cette fonction
